...
pub fn mod_current_apk(
    ...
    let obb_backups =
        save_obbs(Path::new(*(paths::OBB_DIR.get().unwrap())), &obb_backup).context("Saving OBB files")?;
...
pub fn downgrade_and_mod_apk(
    ...
    for obb_diff in &diffs.obb_diffs {
        let obb_path = Path::new(*(paths::OBB_DIR.get().unwrap())).join(&obb_diff.file_name);
        if !obb_path.exists() {
            return Err(anyhow!(
    ...
    let contains_dlc = has_file_with_no_extension(*(paths::OBB_DIR.get().unwrap())).context("Checking for DLC")?;
...
pub fn kill_app() -> Result<()> {
    ...
    Command::new("am").args(&["force-stop", *(APK_ID.get().unwrap())]).output()?;
    Ok(())
}
...
fn patch_and_reinstall(
    ...
    if Path::new(*(paths::PLAYER_DATA.get().unwrap())).exists() {
    ...
    if Path::new(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap())).exists() {
        info!("Fixing colour schemes in backed up PlayerData.dat");
        match fix_colour_schemes(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap())) {
            Ok(_) => {}
    ...
    restore_obb_files(Path::new(*(paths::OBB_DIR.get().unwrap())), obb_paths).context("Restoring OBB files")?;
...
pub fn backup_player_data() -> Result<()> {
    info!("Copying to {}", *(paths::AUX_DATA_BACKUP.get().unwrap()));

    std::fs::create_dir_all(Path::new(*(paths::AUX_DATA_BACKUP.get().unwrap())).parent().unwrap())?;
    std::fs::copy(*(paths::PLAYER_DATA.get().unwrap()), *(paths::AUX_DATA_BACKUP.get().unwrap()))?;

    if Path::new(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap())).exists() {
        warn!("Did not backup PlayerData.dat to datakeeper folder as there was already a PlayerData.dat there.
            The player data is still safe in {}", *(paths::AUX_DATA_BACKUP.get().unwrap()));
    } else {
        info!("Copying to {}", *(paths::DATAKEEPER_PLAYER_DATA.get().unwrap()));
        std::fs::create_dir_all(Path::new(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap())).parent().unwrap())?;
        std::fs::copy(*(paths::PLAYER_DATA.get().unwrap()), *(paths::DATAKEEPER_PLAYER_DATA.get().unwrap()))?;
    }
...
fn reinstall_modded_app(temp_apk_path: &Path) -> Result<()> {
    ...
    Command::new("pm")
        .args(["uninstall", *(APK_ID.get().unwrap())])
        .output()
    ...
    Command::new("appops")
        .args(["set", "--uid", *(APK_ID.get().unwrap()), "MANAGE_EXTERNAL_STORAGE", "allow"])
        .output()?;
...
fn save_libunity(
    ...
    let url = match external_res::get_libunity_url(res_cache, *(APK_ID.get().unwrap()), version)? {
        Some(url) => url,
...
pub fn get_modloader_path() -> Result<PathBuf> {
    let modloaders_path = format!("/sdcard/ModData/{}/Modloader/", *(APK_ID.get().unwrap()));
...
pub fn check_obb_present() -> Result<bool> {
    if !Path::new(*(paths::OBB_DIR.get().unwrap())).exists() {
        return Ok(false);
    }

    // Check if any of the files in the OBB directory have extension OBB
    Ok(std::fs::read_dir(*(paths::OBB_DIR.get().unwrap()))?.any(|stat_res| {
