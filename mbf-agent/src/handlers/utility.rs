...
pub(super) fn handle_fix_player_data() -> Result<Response> {
    ...
    if Path::new(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap())).exists() {
        ...
        data_fix::fix_colour_schemes(*(paths::DATAKEEPER_PLAYER_DATA.get().unwrap()))?;
        did_work = true;
    }
    ...
    if Path::new(*(paths::PLAYER_DATA.get().unwrap())).exists() {
        ...
        debug!("(removing {})", *(paths::PLAYER_DATA.get().unwrap()));
        std::fs::remove_file(*(paths::PLAYER_DATA.get().unwrap())).context("Deleting faulty player data")?;
        if Path::new(*(paths::PLAYER_DATA_BAK.get().unwrap())).exists() {
            std::fs::remove_file(*(paths::PLAYER_DATA_BAK.get().unwrap()))?;
        }
        did_work = true;
    } else {
