...
use crate::{downloads, mod_man::ModManager, requests::{self, Request, Response}};
...
pub fn handle_request(request: Request) -> Result<Response> {
    match request {
        Request::GetModStatus { override_core_mod_url } => mod_status::handle_get_mod_status(override_core_mod_url),
        Request::Patch { downgrade_to ,
            ...
            vr_splash_path} => 
            patching::handle_patch(downgrade_to, remodding, manifest_mod, allow_no_core_mods, override_core_mod_url, vr_splash_path),
        Request::GetDowngradedManifest { version } => patching::handle_get_downgraded_manifest(version),
        ...
        Request::QuickFix { override_core_mod_url, wipe_existing_mods } => utility::handle_quick_fix(override_core_mod_url, wipe_existing_mods)
    }
}
...
fn install_core_mods(res_cache: &ResCache,
    mod_manager: &mut ModManager,
    app_info: requests::AppInfo,
    override_core_mod_url: Option<String>) -> Result<()> {
