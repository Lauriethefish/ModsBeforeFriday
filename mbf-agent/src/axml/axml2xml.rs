...
pub fn axml_to_xml<W: std::io::Write,R: std::io::Read + std::io::Seek>
    ...
    while let Some(ev) = reader.read_next_event()? {
        match ev {
            AxmlEvent::StartElement { attributes, name, namespace, line_num: _ } => {
                ...
                for (attr, string_value) in attributes.iter().zip(string_attr_values.iter()) {
                    builder = builder.attr(get_xml_name_from_axml(&attr.name, &attr.namespace, &current_ns_prefixes), &string_value);
            ...
            AxmlEvent::EndElement { line_num: _, namespace, name } => 
                writer.write(XmlEvent::end_element()
                    ...
                    .name(get_xml_name_from_axml(&name, &namespace, &current_ns_prefixes))),
    ...
    loop {
        match reader.next()? {
            ...
            XmlEvent::StartElement { name, attributes, namespace } => {
                ...
                let newly_declared_nses: Vec<AxmlNamespace> = ns_map.iter()
                    .filter(|(prefix, uri)| 
                    ...
                    .map(|(prefix, uri)| AxmlNamespace {
                        ...
                        prefix: if prefix == xml::namespace::NS_NO_PREFIX { None } else { Some(prefix.clone()) },
                        uri: uri.clone()
                    }).collect();

                // Write a StartNamespace chunk for each newly declared namespace and add it to the map
                for ns in &newly_declared_nses {
                    current_namespace_map.insert(match &ns.prefix {
...
fn get_xml_name_from_axml<'a>(name: &'a String,
    namespace: &'a Option<String>,
    // Map of namespace URIs TO namespace prefixes.
    ns_prefixes: &'a HashMap<String, String>) -> XmlName<'a> {
    match namespace {
        // Use the correct namespace prefix from the provided map
        Some(ns_uri) => XmlName::qualified(&name, &ns_uri, 
...
fn stringify_attr_value(value: AxmlAttrValue) -> String { 
    match value {
        AxmlAttrValue::Boolean(b) => if b { "true".to_string() } else { "false".to_string() },
...
fn attr_value_from_string(string: String) -> Result<AxmlAttrValue> {
    Ok(if string == "true" {
        ...
        AxmlAttrValue::Reference(string[5..string.len() - 1].parse::<u32>()
