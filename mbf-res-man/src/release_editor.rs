...
pub fn get_assets(release: &Release, auth_token: &str) -> Result<Vec<ReleaseAsset>> {
    let req_path = format!(
        "{API_ROOT}/repos/{}/{}/releases/{}",
        release.repo.owner, release.repo.repo, release.id
    );
...
pub fn delete_asset(asset: &ReleaseAsset, release: &Release, auth_token: &str) -> Result<()> {
    let req_path = format!(
        ...
        release.repo.owner, release.repo.repo, asset.id
    );

    set_headers(
        crate::default_agent::get_agent().delete(&req_path),
        auth_token,
    )
    .call()?;
...
pub fn upload_asset_from_reader(
    release: &Release,
    file_name: &str,
    ...
    let req_path = format!(
        "{UPLOAD_API_ROOT}/repos/{}/{}/releases/{}/assets",
        release.repo.owner, release.repo.repo, release.id
    );
...
pub fn upload_or_overwrite(
    file_path: impl AsRef<Path>,
    assets: &[ReleaseAsset],
    crc32_map: &mut HashMap<String, u32>,
    release: &Release,
    auth_token: &str,
    hash_cache: &mut HashCache<u32>,
) -> Result<()> {
    let file_name = file_path
        .as_ref()
        .file_name()
        .ok_or(anyhow!("Asset had no filename"))?
        .to_string_lossy()
        .to_string();
...
pub fn read_existing_asset_crc32s(
    release_files: &[ReleaseAsset],
    auth_token: &str,
) -> Result<(Option<ReleaseAsset>, HashMap<String, u32>)> {
    match release_files
        .iter()
        .filter(|asset| asset.file_name == ASSET_CRC_FILENAME)
        .next()
    {
        Some(crc32_asset) => {
...
pub fn write_asset_crc32s(
    release: &Release,
    asset_crc32s: &HashMap<String, u32>,
    auth_token: &str,
) -> Result<()> {
...
pub fn update_release_from_directory(
    ...
    for file in list_files_json_last(&dir_path).context("Listing release files")? {
        ...
        if let Err(err) = upload_or_overwrite(
            &path,
            &release_files,
            &mut crc32_map,
            release,
