...
fn extract_access_token(access_token_result: &serde_json::Value) -> Result<String> {
    Ok(access_token_result["access_token"]
        .as_str()
        .ok_or(anyhow!("access_token was not a string"))?
        .to_string())
}
...
fn get_horizon_access_token(access_token: &str) -> Result<String> {
    ...
    let access_token_result = &json_doc["data"]["xfr_create_profile_token"]["profile_tokens"]
        .as_array()
        .ok_or(anyhow!("profile_tokens was not an array!"))?[0];
...
pub fn get_quest_access_token(email: &str, password: &str) -> Result<String> {
    ...
    let horizon_access_token =
        get_horizon_access_token(&access_token).context("Getting horizon token")?;
...
pub fn get_obb_binary(access_token: &str, android_binary_id: &str) -> Result<Option<ObbBinary>> {
    let resp = crate::default_agent::get_agent()
        ...
        .send_form(&[
            ...
            (
                "variables",
                &format!("{{\"binaryID\":\"{android_binary_id}\"}}"),
            ),
        ])?;
