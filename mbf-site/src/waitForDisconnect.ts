...
export async function waitForDisconnect(device: Adb) {
    if (disconnectPromises.has(device.serial)) {
        Log.debug(`Already waiting for ${device.serial} to disconnect`);
        return await disconnectPromises.get(device.serial);
    }

    Log.debug(`Waiting for ${device.serial} to disconnect`);
    ...
    if (disconnectedEarly) {
        try {
            Log.debug(`Waiting for ${device.serial} to disconnect using subprocess`);
    ...
    Log.debug(`Devoce ${device.serial} disconnected`);
    resolver.resolve();
